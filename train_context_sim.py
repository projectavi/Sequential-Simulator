...
class KarpovEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.rating_fc1 = nn.Linear(2, 64, bias=True)
        self.rating_fc2 = nn.Linear(64, 256, bias=True)
        self.rating_fc3 = nn.Linear(256, 768, bias=True)
        self.act = nn.ReLU()

        self.tr_emb = nn.Linear(768, 768, bias=True)
    ...
    def forward(
        self, input_ids=None, ratings=None, token_type_ids=None, position_ids=None, inputs_embeds=None, past_key_values_length=0
    ):
        ...
        r = self.rating_fc1(ratings.float())
        r = self.act(r.float())
        r = self.rating_fc2(r.float())
        r = self.act(r.float())
        r = self.rating_fc3(r.float())
        ...
        r = r.reshape((embeddings.shape[0], 1, embeddings.shape[2]))
        rating_embeddings = r.repeat(1, embeddings.shape[1], 1)
        embeddings += rating_embeddings
        embeddings = self.act(self.tr_emb(embeddings))
...
class KarpovEncoder(RobertaEncoder):
    def __init__(self, config):
